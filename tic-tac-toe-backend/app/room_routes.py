...
def create_room():
    ...
    while Game.query.filter_by(room_id=room_id).first(): # Ensure room_id is unique
        room_id = generate_room_code()
    ...
    return jsonify({
        ...
        "game_id": new_game.id,
        "game_details": new_game.to_dict(current_user_id=current_user_id)
    }), 201
...
@jwt_required()
def join_room_http(room_id_param): # Renamed to avoid conflict with socket event
    current_user_id = get_jwt_identity()
    user = User.query.get(current_user_id)
    if not user:
        return jsonify({"msg": "User not found"}), 404
    ...
    if game.status != 'pending':
        return jsonify({"msg": "Room is not available for joining (already active or finished)"}), 403
    
    if game.player_x_id == current_user_id:
         # User is already player X, perhaps rejoining or checking status
        return jsonify({"msg": "You are already Player X in this room.", "game_details": game.to_dict(current_user_id)}), 200
    ...
    if game.player_o_id is None:
        ...
        if player_x_sid:
            socketio.emit('player_joined', {'game': game.to_dict(current_user_id=game.player_x_id), 'joining_player_username': user.username}, room=player_x_sid)
        
        # Notify the joining player (Player O)
        # The client joining will typically then connect to the socket room for this game
        return jsonify({"msg": "Joined room successfully as Player O. Game is active.", "game_details": game.to_dict(current_user_id)}), 200
    
    # If user is already player O
    if game.player_o_id == current_user_id:
        return jsonify({"msg": "You are already Player O in this room.", "game_details": game.to_dict(current_user_id)}), 200
...
def set_ready_to_play():
    ...
    if current_user_id not in ready_to_play_users:
        ...
        socketio.emit('available_players_update', list_available_players_data(), room='lobby') # Assuming a general lobby room
...
def set_unready_to_play():
    ...
    if current_user_id in ready_to_play_users:
        ...
        socketio.emit('available_players_update', list_available_players_data(), room='lobby')
    return jsonify({"msg": "No longer marked as ready to play."}), 200

def list_available_players_data():
    current_user_id = get_jwt_identity() if get_jwt_identity() else None # Handle anonymous access
    # Return list of {"id": user_id, "username": username}
    return [
        {"id": uid, "username": uname} 
        for uid, uname in ready_to_play_users.items() 
        if uid != current_user_id # Don't list self
    ]

@room_bp.route('/play/available', methods=['GET'])
@jwt_required(optional=True) # Allow even non-logged in users to see, or only logged in. Let's make it required.
@jwt_required()
def list_available_players():
    return jsonify(list_available_players_data()), 200
...
def start_game_with_player(opponent_id):
    ...
    if challenger_id not in ready_to_play_users and opponent_id not in ready_to_play_users:
        ...
        return jsonify({"msg": "Both players must be marked as ready to play, or one is no longer available."}), 400
    ...
    new_game = Game(
        ...
        is_public=False, # Games started this way are not listed as 'public waiting rooms'
        status='active'
    )
    db.session.add(new_game)
    db.session.commit()
    ...
    if challenger_id in ready_to_play_users: del ready_to_play_users[challenger_id]
    if opponent_id in ready_to_play_users: del ready_to_play_users[opponent_id]
    socketio.emit('available_players_update', list_available_players_data(), room='lobby') # Update global list
    ...
    if challenger_sid: # Also notify challenger to join the socket room
        socketio.emit('game_started_direct', { # A specific event for this type of start
    ...
    return jsonify({
        "msg": f"Game started with {opponent.username}",
        "game_details": new_game.to_dict(current_user_id=challenger_id)
    }), 201